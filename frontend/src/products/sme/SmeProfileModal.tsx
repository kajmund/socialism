...
export function SmeProfileModal({
  open,
  onClose,
}: {
  open: boolean
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="sme-profile-modal-title"
      onClick={(event) => {
        if (event.target === event.currentTarget) onClose()
      }}
    >
      <div className="flex h-[min(92vh,820px)] w-full max-w-2xl flex-col overflow-hidden rounded-[var(--radius-lg)] border border-white/15 bg-db-ink-0 shadow-2xl">
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-[color:var(--border-hairline)] bg-db-ink-950 px-4 text-db-ink-0">
          ...
          <h2 id="sme-profile-modal-title" className="text-sm font-medium">
            {t("profile.title")}
          </h2>
          <AdminButton
            variant="accent"
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <ProfilePage embedded Shell={SmeEmbedPanelShell} />
        </div>
      </div>
    </div>
  )
