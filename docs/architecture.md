...
Statuses on Attempt: `created`, `researching`, `ready`, `running`, `completed`, `failed`. `execute_attempt_research` (`app/services/research/execution.py`) claims `created → researching`, seeds one `ResearchNeedExecution` per initial plan need (`pending → running → completed/failed`), bounds concurrent retrieval with `research_need_concurrency` (claim and persist stay outside that limit), and persists each need's `found` / `not_found` / `error` items when that need finishes. The EvidenceSet stays `building` until every initial need execution is terminal; then a `ResearchAssessment` judges the persisted EvidenceSet against the ResearchPlan (`sufficient` / `insufficient`). Insufficient is a valid v1 outcome — the set still freezes and the Attempt becomes `ready`. Assessor/model/parsing failure is fail-closed: EvidenceSet and Attempt are marked `failed` without freezing. Tenant scope always comes from the Run. Source-level errors complete that need and do not fail the Attempt; worker or orchestration failure marks EvidenceSet `failed` and Attempt `failed` without freezing.
