...
- **Supabase:** one project owns durable product state, Auth, private files, reports and research vectors. Server-only storage and vector operations share the backend service-role credential.
...
| **CanonicalDocument** | Citable source identity (upload, later lagen.nu / APIs). Deduplicated on source identity. |
| **DocumentSection** | Hierarchical structural region inside a document. |
| **TextUnit** | Canonical passage. Embeddings, Q&A, and later claims/research ground here. |
...
Platform-level lifecycle in `backend/app/services/execution/`. An **ExecutionRun** is a long-lived investigation (mandatory `customer_id`, plus `module`, `title`, `context`). An **ExecutionAttempt** is one execution: `configuration_snapshot`, `input_snapshot`, `research_objective_snapshot`, and `research_plan_snapshot` are historical JSON, not pointers to live config. An **EvidenceSet** snapshots `ResearchEvidence` into `evidence_set_items` (excerpt, locator, provenance, original evidence id, ordinal, content hash) and becomes immutable when frozen. `clone_attempt` (and `POST /execution/attempts/{id}/clone`) creates a new `ready` Attempt with `parent_attempt_id`, the same frozen `evidence_set_id`, copied input/research snapshots, and no result. Optional `configuration_snapshot` is a full replacement — no merge. Source Attempt and EvidenceSet are never mutated. Clone is not idempotent.

Statuses on Attempt: `created`, `researching`, `ready`, `running`, `completed`, `failed`. `execute_attempt_research` (`app/services/research/execution.py`) first resolves an immutable Attempt-level `research_objective_snapshot` and an injectable `ResearchPlanner` into a validated initial `research_plan_snapshot` (explicit `ResearchPlan` remains a test/internal path). The planner is only offered evidence natures the attempt's capability registry can execute (`ResearchRouter.available_source_types` when a router is passed; otherwise the same standard capability descriptors that `build_standard_research_router` registers, introspected without a session-bound router); a generated empty plan is invalid. Planner/model/invalid-plan failure leaves the Attempt `created`. It then claims `created → researching`, writes `research_runtime_needs` for the initial wave, and seeds one `ResearchNeedExecution` per need (`pending → running → completed/failed`). Retrieval is bounded by `research_need_concurrency` (claim and persist stay outside that limit). After each wave barrier an immutable `research_evidence_quality` row is persisted per item (declared authority / recency / source nature / corroboration, optional injectable relevance). Then a `ResearchAssessment` pass judges the **accumulated** EvidenceSet against the original snapshot. That is **local sufficiency**: whether the questions already on the runtime inventory are supported. Quality is inspectable; v1 does not discard evidence or flip sufficiency from an aggregate score. See [research-evidence-quality.md](guides/research-evidence-quality.md). `insufficient` asks an injectable `FollowUpResearchPlanner` for novel `ResearchNeed`s, persists them (lineage + wave) **before** the next retrieval wave, and reassesses. When the local loop is terminal (`no pending needs` + latest assessment `sufficient`), an injectable `ResearchCompletenessReviewer` judges the **original persisted research objective**. That is **global completeness**: a locally well-supported plan can still have omitted a material question. The reviewer is offered the same executable source types as the planner (`ResearchRouter.available_source_types()` when a router is passed; otherwise the same session-independent standard capability descriptors). A material omitted question may still be identified; only executable source types become runnable `ResearchNeed`s. Catalog types the capability registry cannot run are an explicit `capability_unavailable` gap, not a novel follow-up. `complete` is the freeze barrier. `incomplete` persists candidate questions as `origin=global_completeness` runtime needs (source pass + rationale) through the same validate/dedupe/budget/retrieval machinery, then reassesses and gates again. The EvidenceSet stays `building` across waves. The loop stops on local+global complete (`stop_reason=sufficient` plus a persisted complete pass), `max_iterations`, `max_needs`, `no_novel_followups`, `max_completeness_passes`, or `capability_unavailable` and only then freezes + `ready`. Budget exhaustion while globally incomplete is a completed research outcome that still exposes the latest incomplete pass and the stop reason. Assessor/planner/completeness-reviewer/model/parsing/worker failure is fail-closed: EvidenceSet and Attempt are marked `failed` without freezing. See [research-completeness.md](guides/research-completeness.md). Tenant scope always comes from the Run. Source-level errors complete that need and do not fail the Attempt. `ResearchRouter` resolves each `ResearchNeed` through a KnowledgeProvider capability registry: declared constraints (v1: `source_types` as evidence natures, plus optional domain/modality/capability filters) select a bounded provider set programmatically. MCP/API/vector-store/dataset are access metadata, not top-level source types. Unknown or unmatched capabilities are explicit `error` / `not_found` — never a silent fallback. See [knowledge-provider-registry.md](guides/knowledge-provider-registry.md). Before provider retrieval, `execute_attempt_research` may resolve a runtime need to a persistent `KnowledgeQuestion` and do a bounded one-hop `ANSWERED_BY` lookup. Candidates are normalized into `ResearchEvidence` and re-enter the existing EvidenceSet + quality + local sufficiency path; they cannot mark a need sufficient by themselves. Graphiti is adapter-only. See [research-question-evidence.md](guides/research-question-evidence.md).
