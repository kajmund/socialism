...
## Document-structure diagnosis

A read-only comparison of the public HTML headings with MCP retrieval established that the actual special-commentary heading is OCR-labelled `1 Specialmotivering` (`#a1-6`). MCP retrieves only 463 characters there, ending after the introductory paragraph under 7.1. The substantive commentary starts at the next same-level OCR heading, `36 S` (`#a36-2`, 32,936 characters). The later contents-table entry `#a7-4` is only 305 characters and genuinely is a contents listing. Both anchors were discovered in the published document; none were guessed.

Other search fragments carrying the number 36 were proposed wording (`#a36-3`) or Lagråd material (`#a36-4`). Thus paragraph number and title alone do not establish the requested text role. The production adapter has not been changed to guess anchors or use an alternate HTML-retrieval fallback. The confirmed remaining retrieval requirement is to retain document outline / preceding section context through MCP passage resolution so that the actual commentary can be located and attributed together. A diagnostic manual fetch is not proof that the automatic provider solves the question.
