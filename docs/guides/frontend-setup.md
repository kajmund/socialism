...
Smoke-test: switch to English in the admin top nav — `/jobs`, `/runs`, `/populations`, and `/runs/new` (wizard + tick modal) should flip labels; refresh should keep English. New strings: add to `sv.ts` first, mirror in `en.ts`, then call `t("…")`. See `frontend/AGENTS.md`.
