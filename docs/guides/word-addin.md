...
2. **Granska** reads `context.document.body.paragraphs` (and `uniqueLocalId` when WordApi 1.6 is present), groups sections on Heading 1 / Rubrik 1, and `POST /expertgranskning/word-jobs` with every raw paragraph plus optional session-local ids. The server derives deterministic text/context hashes from that frozen snapshot. Headings get the same anchor fields as body paragraphs.
...
5. Comments and rewrites use the same resolver (`resolveWordAnchor`): local id + text, then original index + exact text, then unique exact text, then previous/next hashes. Fail closed on `stale` / `ambiguous` / `missing` — no Word mutation. Before mutating, the add-in claims `pending → applying` with an `application_id`, then completes `applying → applied` (or marks `unresolved`). Replay of `applying` / `applied` / `unresolved` never auto-applies again. Rewrites still use Track Changes when available.
