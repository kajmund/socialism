...
5. First run writes `socialism_doc_id` to `Office.context.document.settings` and sends it as `doc_id`. Later **Granska** calls `GET /expertgranskning/word-jobs/latest?doc_id=…`, resolves previous Word comments, then starts a new job.
