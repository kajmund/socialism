...
A second replay including derived legal-analysis metadata was prepared locally but not sent: automatic approval review requires additional user authorization for that broader external transfer. Local preparation loaded 47 structured legal results; their bounded context increased input size, reducing represented evidence to 41 of 54 under current budgets. Any future comparison must report these clipping differences explicitly. Backend `/health` returned `ok`; PR #297's updated concurrency commit passed CI.
