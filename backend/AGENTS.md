...
**Playground:** `GET /playground/anchors`, `POST /playground/ssr/rate`, `POST /playground/ssr/compare`, `POST /playground/prompts/run`, `POST /playground/image/react` (multipart: persona + image → vision description, in-character reaction, SSR tone/style) — ephemeral calibration; no persistence.
