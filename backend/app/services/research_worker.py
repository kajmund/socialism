...
async def run_research_claim(attempt_id: str) -> None:
    ...
    heartbeat = asyncio.create_task(
        _heartbeat_lease(attempt_id, lease_token=lease_token or "", stop=stop)
    )
    try:
        await _execute_claimed_research(
            factory,
            attempt_id=attempt_id,
...
async def _heartbeat_lease(
    attempt_id: str, *, lease_token: str, stop: asyncio.Event
) -> None:
    ...
    while not stop.is_set():
        try:
            ...
            pass
        async with factory() as session:
            await renew_research_lease(session, attempt_id, lease_token=lease_token)
            await session.commit()
