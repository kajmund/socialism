...
async def generate_rattsunderlag_module_report(
    ...
    source_id = str(sources[0].get("session_id") or "").strip()
    if not source_id:
        raise ValueError("rattsunderlag source requires session_id")
    async with ctx.session_factory() as session:
        job = await get_research_job(session, source_id)
        if job is None:
            raise ValueError(f"Research job not found: {source_id}")
        job_id = job.id
        raw = job.result if isinstance(job.result, dict) else None
        payload = raw.get("result") if raw else None
        if not isinstance(payload, dict):
            raise ValueError(f"Research job {job.id} has no result payload")
