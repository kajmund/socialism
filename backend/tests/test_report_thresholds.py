...
def test_recommendation_ssr_block_uses_configured_thresholds():
    from app.services.report.bundles import RunBundle
    from app.services.report.classify import BundleClassification, TopicPack
    from app.services.report.metrics import compute_report_metrics
    ...
    metrics = compute_report_metrics([bundle], [clf])
    loose = default_report_thresholds().model_copy(
        update={
            "recommendation": default_report_thresholds().recommendation.model_copy(
                update={
                    "action_bands": default_report_thresholds()
    ...
    default_block = build_recommendation_ssr_block(
        metrics, [bundle], [clf], locale="sv", thresholds=default_report_thresholds()
    )
    loose_block = build_recommendation_ssr_block(
        metrics, [bundle], [clf], locale="sv", thresholds=loose
    )
    assert default_block["action"] == "Publicera efter mindre justeringar"
    assert loose_block["action"] == "Redo att publicera"
